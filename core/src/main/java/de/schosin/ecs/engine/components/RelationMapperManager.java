...
public class RelationMapperManager implements RemovedRelationTypeHandler, EntitiesDeletedObserver {
    ...
    public RelationMapperManager(StorageEngine engine, BagManager bagManager, ComponentManager componentManager, TransmutationManager transmutationManager) {
        this.engine = engine;
        ...
        for (int i = components.getSize() - 1; i >= 0; i--) {
            ...
            if (component instanceof EntityRelationComponent<?, ?> relationComponent) {
                for (int e = 0, es = entities.getSize(); e < es; e++) {
                    relationComponent.removeTarget(entities.get(e), this);
                }
            }
        }
    }
